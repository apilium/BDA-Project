...
First use priors that we picked
```{r}
prior.fit.feature_selected.basic <- brm(formula = time | cens(DEATH_EVENT) ~ age  + anaemia + high_blood_pressure,
           data = train.data,
           family = weibull(),
           prior = c(set_prior("cauchy(40, 20)", class="b", coef="age"),
                     prior_string("normal(.5, .5)", class="b", coef="anaemia"),
...
death.events <- train.data$time
yrep <- posterior_predict(prior.fit.feature_selected.basic, draws=length(death.events))
ppc_dens_overlay(y=death.events, yrep = yrep)

pp_check(prior.fit.feature_selected.basic)
```


Then use normal priors
```{r}
prior.fit.feature_selected.uninfo <- brm(formula = time | cens(DEATH_EVENT) ~ age  + anaemia + high_blood_pressure,
           data = train.data,
           family = weibull(),
           prior = set_prior('normal(0,1000)'),
           refresh=0)
death.events <- train.data$time
yrep <- posterior_predict(prior.fit.feature_selected.uninfo, draws=length(death.events))
ppc_dens_overlay(y=death.events, yrep = yrep)
```
