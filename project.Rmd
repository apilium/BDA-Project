...
fit.full <- brm(formula = DEATH_EVENT ~ age + ejection_fraction + serum_creatinine + serum_sodium + high_blood_pressure + creatinine_phosphokinase + diabetes + smoking + anaemia,
                prior = c(set_prior("normal(0,50)", class="b", coef="age"),
                          set_prior("normal(0,100)", class="b", coef="serum_creatinine"),
                          set_prior("normal(0,100)", class="b", coef="ejection_fraction"),
                          set_prior("normal(0,1000)", class="b", coef="serum_sodium"),
...
fit.hierarchical <- brm(formula = DEATH_EVENT ~ ejection_fraction + serum_creatinine + serum_sodium + (ejection_fraction + serum_creatinine + serum_sodium| age),
           data = train.data,
           family = bernoulli(),
           c(set_prior("normal(0,100)", class="b", coef="serum_creatinine"),
              set_prior("normal(0,100)", class="b", coef="ejection_fraction"),
              set_prior("normal(0,1000)", class="b", coef="serum_sodium")),
           refresh=0
)
...
```{r}
prs <- prior_summary(brm(formula = DEATH_EVENT ~ age + ejection_fraction + serum_creatinine + serum_sodium + high_blood_pressure + creatinine_phosphokinase + diabetes + smoking + anaemia,
           data = train.data,
           family = bernoulli()))
```
...
```{r}
check_depthtree(fit.feature_selected$fit)
```
...
```{r}
check_depthtree(fit.hierarchical$fit)
```
...
```{r}
check_depthtree(fit.weibull$fit)
```
...
```{r}
pp_check(fit.feature_selected, newdata = test.data)
```

```{r}
pp_check(fit.full, newdata = test.data)
```

```{r}
pp_check(fit.hierarchical, newdata = test.data)
```

```{r}
pp_check(fit.weibull, newdata = test.data)
```
